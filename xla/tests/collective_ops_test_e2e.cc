...
class AsyncCollectiveOps : public CollectiveOpsTestE2E,
 ...
 protected:
  ...
  bool IsAsync(const HloInstruction* instr) {
    ...
    return !instr->backend_config<gpu::GpuBackendConfig>()
                .value()
...
XLA_TEST_P(AsyncCollectiveOps, AsyncAllReduce) {
  ...
  if (enable_async_all_reduce){
    VLOG(-1) << "enable_async_all_reduce is TRUE";
  }
  else{
    VLOG(-1) << "enable_async_all_reduce is FALSE";
  }
