...
template < size_t K, typename KT, typename VT>
__device__ void topk_kernel_main(KT* data, int n, KT* result, 
                      uint32_t* result_idxs, int k) 
{
...
{
  topk_kernel_main< K, KT, VT >(data, n, result, result_idxs, k);
}
...
void* GetTopKKernelForK(int n_threads) {

  return reinterpret_cast<void*>
      (n_threads <= 512 ? &Run512<K, T, uint32_t> : &Run1024<K, T, uint32_t>);
}
