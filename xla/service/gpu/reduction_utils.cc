...
bool IsUnnestedReductionFasterThanElemental(
  ...
  if (reduction_dimensions.is_row_reduction) {
    ...
    return (reduction_dimensions.dimensions[2] >= 32) ||
           ((32 % reduction_dimensions.dimensions[2]) == 0);
  }
  ...
  bool prefer_elemental_emitter =
      (major_size < 32) ||
      (major_size < 2 * 32 && minor_size < 32) ||
      (major_size < 4 * 32 && minor_size < 8) ||
      (major_size < 8 * 32 && minor_size < 3);
...
ReductionDimensions GetReductionKindAndContiguousComponents(
  ...
  absl::Span<const int64_t> dims_to_reduce = reduce.dimensions();
  VLOG(2) << "dims_to_reduce "
           << " [" << dims_to_reduce[0] << " " << dims_to_reduce[1] << " "
          << dims_to_reduce[2] << "]";
  DimensionVector dims_to_keep;
