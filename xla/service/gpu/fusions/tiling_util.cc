...
absl::StatusOr<TilingThreadIdInfo> EmitThreadIdInfo(llvm::IRBuilder<>* builder,
  ...
  info.lane_id =
      builder->CreateURem(info.thread_id, constant(wpsize), "lane_id");
  return info;
}
...
absl::StatusOr<TilingKernelInfo> EmitTilingKernel(
    ...
    const TileGenerator& tile_generator, const int wpsize) {
  ...
  TF_ASSIGN_OR_RETURN(TilingThreadIdInfo thread_id_info,
                      EmitThreadIdInfo(builder, tiling, index_ty, wpsize));
