...
absl::Status CreateTritonPipeline(
  ...
  pm.addPass(mt::createConvertTritonToTritonGPUPass(
      absl::StrCat("hip:", ccRocm.gfx_version()), config.num_warps,
      threadsPerWarp, config.num_ctas));
  ...
  if(config.num_stages == 0 and ccRocm.has_amd_matrix_core()) {
  ...
  pm.addPass(mt::createConvertTritonAMDGPUToLLVMPass(ccRocm.gfx_version()));
  pm.addPass(mlir::createCanonicalizerPass());
  pm.addPass(mlir::createCSEPass());
