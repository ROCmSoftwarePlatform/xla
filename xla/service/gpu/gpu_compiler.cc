...
absl::Status GpuCompiler::OptimizeHloModule(
  ...
  {
    VLOG(-1) << "start to collective-optimizations....";
    ...
    TF_RETURN_IF_ERROR(collectives_pipeline.Run(hlo_module).status());
      ...
      VLOG(-1) << "complete collective-optimizations....";
...
GpuCompiler::CompileToTargetBinary(const HloModuleConfig& module_config,
                                   ...
                                   const CompileOptions& options,
                                   const HloModule* debug_module) {
  VLOG(-1) << "start to GpuCompiler::CompileToTargetBinary()...";
  ...
  VLOG(-1) << "start to maybe_backend_result...";
...
GpuCompiler::CompileToBackendResult(
    ...
    const se::DeviceDescription& gpu_device_info) {
  VLOG(-1) << "start GpuCompiler::CompileToBackendResult()...";
  TF_ASSIGN_OR_RETURN(
  ...
  VLOG(-1) << "start CompileModuleToLlvmIr()...";
  // Compile the module
  TF_ASSIGN_OR_RETURN(
  ...
  VLOG(-1) << "start CompileToTargetBinary()...";
  TF_ASSIGN_OR_RETURN(
...
absl::StatusOr<std::unique_ptr<Executable>> GpuCompiler::RunBackend(
    std::unique_ptr<HloModule> module, se::StreamExecutor* stream_exec,
    const CompileOptions& options) {
  VLOG(-1) << "start to GpuCompiler::RunBackend()...";
  const DebugOptions& debug_opts = module->config().debug_options();
  TF_ASSIGN_OR_RETURN(TargetConfig gpu_target_config,
                      GetTargetConfig(options, debug_opts, stream_exec));
  ...
  VLOG(-1) << "start to CompileToBackendResult()...";
  TF_ASSIGN_OR_RETURN(
