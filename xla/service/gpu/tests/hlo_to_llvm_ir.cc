...
When passed the parameter `--ptx`, the LLVM IR will be optimized and PTX/Hsaco
  ...
  if (!generate_ptx) {
    ...
    TF_ASSIGN_OR_RETURN(
        ...
        xla::gpu::nvptx::CompileToPtx(
            llvm_module.get(), gpu_version, hlo_module->config()));
    std::cout << ptx << std::endl;
#elif TENSORFLOW_USE_ROCM
    std::string libdevice_dir = tsl::RocdlRoot();
    xla::gpu::GpuVersion gpu_version{rocm_compute_capability};
    TF_ASSIGN_OR_RETURN(std::vector<uint8_t> hsaco_vec,
                        xla::gpu::amdgpu::CompileToHsaco(
                              llvm_module.get(), gpu_version, 
                              hlo_module->config(), libdevice_dir));
    std::string hsaco_str(hsaco_vec.cbegin(), hsaco_vec.cend());
    std::cout << hsaco_str << std::endl;
#endif
  }
  return xla::OkStatus();
}
