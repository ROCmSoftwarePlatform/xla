...
static hipblasPointerMode_t AsHipblasLtPointerMode(gpu::BlasLt::PointerMode pointer_mode) {
  switch (pointer_mode) {
...
/*static*/ tsl::StatusOr<BlasLt::MatrixLayout> BlasLt::MatrixLayout::Create(
  ...
  //   leading_dim_stride = (order == gpu::MatrixLayout::Order::kRowMajor) ? num_cols : num_rows;
  // }
...
auto BlasLt::MatmulPlan::GetAlgorithms(size_t max_algorithm_count) const ->
...

  {
    ...
    // Wrap hipblas handle immediately, so it is cleaned up if an error occurs.
    Owned<hipblasLtMatmulPreference_t> preference(hip_preference, 
      wrap::hipblasLtMatmulPreferenceDestroy);
...
auto BlasLt::GetMatmulPlan(const gpu::GemmConfig& cfg,
                Epilogue epilogue) const -> tsl::StatusOr< MatmulPlanPtr >
{
  ...
  auto trans_a = blas::Transpose::kNoTranspose,
  ...
  TF_ASSIGN_OR_RETURN(
	...
	if(lhs_layout.order == gpu::MatrixLayout::Order::kRowMajor) {
 	...
 	if(rhs_layout.order == gpu::MatrixLayout::Order::kRowMajor) {
	  rhs_layout.order = gpu::MatrixLayout::Order::kColumnMajor;
...
tsl::Status BlasLt::MatmulPlan::DoMatmul(Stream* stream, const void* alpha, 
...

  {
    ...
    if(auto palgo = std::any_cast< hipblasLtMatmulAlgo_t >(&algorithm.opaque_algo)) {
