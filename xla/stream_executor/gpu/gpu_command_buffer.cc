...
// #if GOOGLE_CUDA
  ...
  TF_RETURN_IF_ERROR(GpuDriver::GraphAddKernelNode(
      ...
      GpuDriver::CreateKernelNodeParams(
...
absl::Status GpuCommandBuffer::LaunchWithPackedArgs(
  ...
  auto params = GpuDriver::CreateKernelNodeParams(gpu_func, blocks.x,
...
absl::Status GpuCommandBuffer::AddNestedCommandBuffer(
  ...
  for(size_t i = 0; i < child_nodes.size(); i++) {
    ...
    if (state_ == State::kUpdate) {
      auto node = execution_scope.
                        nodes[execution_scope.update_state.node_idx++].handle;
      if(isKernel) {
        res = GpuDriver::GraphExecKernelNodeSetParams(exec_, node, 
            "traced kernel", kparams);
      } else {
        res = GpuDriver::GraphExecMemsetNodeSetParams(
          parent_->gpu_context(), exec_, node, mparams);
      }
      ...
      if(i == 0) { // adding the first node of subgraph is enough
        // traced_cache_[node_info.handle] = &nested;
      }
    } // if(state_)
    TF_RETURN_IF_ERROR(res);
  } // for i
  ...
  if (state_ == State::kCreate) {
    ...
    auto res = GpuDriver::GraphAddChildNode(&node_info.handle, graph_, barrier,
                                         child_graph);
    return res;
  }
  return UnsupportedStateError(state_);
...
absl::Status GpuCommandBuffer::Memset(ExecutionScopeId execution_scope_id,
  ...
  auto params = GpuDriver::CreateMemsetNodeParams(
  ...
  if (state_ == State::kCreate) {
    ...
    return GpuDriver::GraphAddMemsetNode(
        parent_->gpu_context(), &node_info.handle, graph_, barrier, params);
  }
  ...
  if (state_ == State::kUpdate) {
    ...
    return GpuDriver::GraphExecMemsetNodeSetParams(
        parent_->gpu_context(), exec_, node, params);
  }
