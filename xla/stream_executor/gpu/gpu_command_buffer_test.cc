...
static Platform* GpuPlatform() {
  auto name = absl::AsciiStrToUpper(xla::PlatformUtil::CanonicalPlatformName("gpu").value());
...
static MultiKernelLoaderSpec GetAddI32KernelSpec() {
  ...
  spec.AddCudaPtxInMemory(stream_executor::gpu::internal::kAddI32Kernel, "add");
#elif defined(TENSORFLOW_USE_ROCM)
  spec.AddCudaCubinInMemory(
    reinterpret_cast<const char*>(&stream_executor::gpu::internal::kAddI32KernelModule[0]),
    "add");
#endif
  return spec;
}
...
using AddI32Ptrs3 = TypedKernel<stream_executor::gpu::internal::Ptrs3<int32_t>>;
...
TEST(GpuCommandBufferTest, LaunchSingleKernel) {
  ...
  MultiKernelLoaderSpec spec(/*arity=*/3);
  spec.AddInProcessSymbol(stream_executor::gpu::internal::GetAddI32Kernel(), "add");
...
TEST(GpuCommandBufferTest, TraceSingleKernel) {
  ...
  MultiKernelLoaderSpec spec(/*arity=*/1, [&](const Kernel& kernel,
    ...
    return PackKernelArgs(add, stream_executor::gpu::internal::Ptrs3<int32_t>{
                                   cast(bufs[0]),
                                   cast(bufs[1]),
                                   cast(bufs[2]),
                               });
  });
  spec.AddInProcessSymbol(stream_executor::gpu::internal::GetAddI32Ptrs3Kernel(), "add");
...
TEST(GpuCommandBufferTest, ConditionalIfElse) {
  ...
  {  // Load multiplication kernel.
    MultiKernelLoaderSpec spec(/*arity=*/3);
    spec.AddInProcessSymbol(stream_executor::gpu::internal::GetMulI32Kernel(), "mul");
...
TEST(GpuCommandBufferTest, ConditionalWhile) {
  ...
  {  // Load inc_and_cmp kernel.
    MultiKernelLoaderSpec spec(/*arity=*/3);
    spec.AddInProcessSymbol(stream_executor::gpu::internal::GetIncAndCmpKernel(), "inc_and_cmp");
